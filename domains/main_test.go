...
`
	...
	assert.Equal(t, "web3.defi.", config.Records[0].Domain)
	assert.Equal(t, "213.189.1.4", config.Records[0].Address)
	assert.Equal(t, uint8(1), config.Records[0].Type)
	assert.Equal(t, uint8(1), config.Records[0].Class)
	assert.Equal(t, uint32(3600), config.Records[0].TTL)

	assert.Equal(t, "www.", config.Records[1].Domain)
	assert.Equal(t, uint8(2), config.Records[1].Type)
	assert.Equal(t, uint8(1), config.Records[1].Class)
